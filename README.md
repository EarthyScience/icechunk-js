...
using zarrita >= 0.7, set `rangeCoalescing: true` to coalesce concurrent reads
against the same backing object. Abort behavior follows zarrita: aborting one
merged read may reject other reads in the same batch.
